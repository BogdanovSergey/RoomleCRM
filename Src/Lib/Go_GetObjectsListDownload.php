<?php
    ...
    $StrCount= 1;
    $ArrStep = 0;
    $CaptArr = array(   '№', 'Добавлен', 'Тип', 'Комнатность', 'Город', 'Метро', 'Улица', 'Этажность', 'Площадь',
        'Цена', 'Выгрузка на сайт', 'Выгрузка в Winner', 'Выгрузка в Cian',
    ...
    while($str  = mysql_fetch_object($res)) {
        ...
        $ValArr = array($ObjData->id,   $ObjData->AddedDate,    $ObjData->ObjectTypeName, $ObjData->RoomsCount, $ObjData->City,
            $ObjData->MetroStation,     $ObjData->StreetHouse,  $ObjData->Floors,   $ObjData->Squares,
            $ObjData->Price,
    ...
    header ( "Content-Disposition: attachment; filename=CityObjects.xls" );
